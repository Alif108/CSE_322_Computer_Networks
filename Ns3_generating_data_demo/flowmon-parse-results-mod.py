...
total_tx = 0;           # total transmitted packets
total_rx = 0;           # total received packets
total_ls = 0;           # total lost packets
...
class Flow(object):
    ...
    __slots_ = ['flowId', 'delayMean', 'packetLossRatio', 'rxBitrate', 'txBitrate',
                ...
                'hopCount', 'flowInterruptionsHistogram', 'rx_duration', 'txPackets', 'rxPackets',
                'lostPacket', 'packetDeliveryRatio']
    ...
    def __init__(self, flow_el):
        ...
        self.packetDeliveryRatio = (rxPackets / (rxPackets + lost))
...
def main(argv):
    ...
    for sim in sim_list:
        for flow in sim.flows:
            ...
            if flow.packetDeliveryRatio is None:
                print("\tPacket Delivery Ratio: None")
            else:
                print("\tPacket Delivery Ratio: %.2f %%" % (flow.packetDeliveryRatio*100))
    
    print("\n")
    print("-------------- Simulation Stats ---------------")
    print("Total Transmitted Packets: %d" % total_tx)
    print("Total Receieved Packets: %d" % total_rx)
    print("Total Lost Packets: %d" % total_ls)
    print("Total Packet Delivery Ratio: %.2f %%" % (total_rx/(total_rx + total_ls)*100))
    print("Total Packet Drop Ratio: %.2f %%" % (total_ls/(total_rx + total_ls)*100))
